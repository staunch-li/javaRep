...
<html lang="en">
		...
		<div class="row">
			<div class="col-12">
				<table class="table table-bordered table-hover text-center">
				  ...
				  <tbody>
					  <c:forEach items="${javamatch}" var="jm">
						    <tr>
						      <!-- <th scope="row"><input type="checkbox" name="single"/></th> -->
						      <td style="display:none" class="jid">${jm.jid }</td>
						      <td>${jm.jname }</td>
							  <td>${jm.project}</td>
						      <td>${jm.jgrade}</td>
							  <td>${jm.jlike}</td>
  ...
  <script type="text/javascript">
	$(".like").click(function() {
		...
		var jid = $tr.children(".jid").text();
		var datas = {"action":"javalike","jid":jid};
		$.ajax({
			   type: "POST",
